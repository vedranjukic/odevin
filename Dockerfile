...
COPY opendevin/frontend/package.json ./package.json
COPY opendevin/frontend/package-lock.json ./package-lock.json
RUN npm install -g npm@10.5.1
RUN npm ci

COPY opendevin/frontend ./
...
COPY opendevin/pyproject.toml opendevin/poetry.lock ./
RUN touch README.md
...
COPY --chown=opendevin:app --chmod=770 opendevin/opendevin ./opendevin
COPY --chown=opendevin:app --chmod=777 opendevin/opendevin/runtime/plugins ./opendevin/runtime/plugins
COPY --chown=opendevin:app --chmod=770 opendevin/agenthub ./agenthub
...
COPY --chown=opendevin:app --chmod=770 opendevin/containers/app/entrypoint.sh /app/entrypoint.sh
...
CMD ["uvicorn", "opendevin.server.listen:app", "--host", "0.0.0.0", "--port", "9000"]